  ...
  PreLoginPacket.prototype.fromBuffer = function(stream, context) {
    ...
    while (stream.readByte() !== 0xFF) {
      ...
      if (context.logDebug) {
        ...
        context.debug('Added pending value type: %d, offset: %d, length: %d', val.type, val.offset, val.length);
      }
    }
    _results = [];
    for (_i = 0, _len = pendingValues.length; _i < _len; _i++) {
      ...
      switch (pendingValue.type) {
        case 0:
          ...
          _results.push(context.debug('Version: ', this.version));
          break;
        case 1:
          this.encryption = stream.readByte();
          _results.push(context.debug('Encryption: ', this.encryption));
          break;
        case 2:
          ...
          stream.skip(1);
          _results.push(context.debug('Instance name: ', this.instanceName));
          break;
        case 3:
          _results.push(context.debug('Ignoring thread ID: '));
          break;
        default:
