var AttentionPacket, BufferBuilder, BufferStream, ColMetaDataToken, DoneToken, Login7Packet, LoginAckToken, Packet, PreLoginPacket, Socket, SqlBatchPacket, StreamIndexOutOfBoundsError, TdsConstants, TokenStreamPacket, fs, util, _ref,
...
exports.TdsClient = (function() {
  ...
  TdsClient.prototype.debug = function(contents) {
    if (this.logDebug) {
      if (!(this._debugFd != null)) this._debugFd = fs.openSync('debug.out', 'w');
      return fs.writeSync(this._debugFd, util.format.apply(null, arguments) + '\n');
    }
  };
  ...
  TdsClient.prototype.connect = function(config) {
    ...
    this.debug('Connecting to SQL Server with config %j', config);
    try {
  ...
  TdsClient.prototype.login = function(config) {
    ...
    this.debug('Logging in with config %j', config);
    try {
  ...
  TdsClient.prototype.sqlBatch = function(sqlText) {
    ...
    this.debug('Executing SQL Batch: %s', sqlText);
    try {
  ...
  TdsClient.prototype._socketConnect = function() {
    ...
    this.debug('Connection established, pre-login commencing');
    try {
  ...
  TdsClient.prototype._socketData = function(data) {
    this.debug('Received %d bytes at state', data.length, this.state, data);
  ...
  TdsClient.prototype._handleToken = function() {
    ...
    while (true) {
      ...
      try {
        ...
        token = null;
        if (this._tokenStreamRemainingLength === 1) {
          this._stream.skip(2);
        } else {
          token = this._tokenStream.nextToken(this._stream, this);
        }
        if (this._tokenStreamRemainingLength - (this._stream.currentOffset() - currentOffset) < 0) {
          this.debug('From %d to %d left negative remaining', currentOffset, this._stream.currentOffset(), this._tokenStreamRemainingLength - (this._stream.currentOffset() - currentOffset));
          this.debug('Buffer at %d', currentOffset, this._stream.getBuffer().slice(currentOffset));
          this._stream.rollbackTransaction();
          this._pendingTokenStreamBuffer = this._stream.readBuffer(this._tokenStreamRemainingLength);
          this.debug('Got pending buffer: ', this._pendingTokenStreamBuffer);
          this._stream.commitTransaction();
          this.debug('What is left: ', this._stream.getBuffer());
          this._handlePacket();
          return;
        } else {
          this._pendingTokenStreamBuffer = null;
        }
        this._tokenStreamRemainingLength -= this._stream.currentOffset() - currentOffset;
        this.debug('From %d to %d offset, remaining: ', currentOffset, this._stream.currentOffset(), this._tokenStreamRemainingLength);
        this._stream.commitTransaction();
      } catch (err) {
        if (err instanceof StreamIndexOutOfBoundsError) {
          this.debug('Stream incomplete, rolling back');
          this._stream.rollbackTransaction();
          return;
        } else {
          if (this._tokenStreamRemainingLength - (this._stream.currentOffset() - currentOffset) < 0) {
            this.debug('Error happened when reading outside of range; ignoring...');
            this._stream.rollbackTransaction();
            this._pendingTokenStreamBuffer = this._stream.readBuffer(this._tokenStreamRemainingLength);
            this._stream.commitTransaction();
            this._handlePacket();
      ...
      if (!this._cancelling || token.type === DoneToken.type) {
        ...
        this.debug('Checking token type: ', token.type);
        switch (token.type) {
    ...
    if (receivedLoginAck) {
      ...
      if ((_ref3 = this._handler) != null) {
        if (typeof _ref3.login === "function") _ref3.login();
      }
    }
    if (this._stream.getBuffer().length > 0) {
      this.debug('More packet to be had, continuing...');
      return this._handlePacket();
    }
  };
  ...
  TdsClient.prototype._handlePacket = function() {
    ...
    try {
      ...
      if (packet instanceof TokenStreamPacket) {
        this.debug('Found token stream packet');
        this._tokenStream = packet;
        this._tokenStreamRemainingLength = header.length - 8;
      } else {
        this.debug('Found non token stream packet');
        packet.fromBuffer(this._stream, this);
      }
      this._stream.commitTransaction();
      if (this._pendingTokenStreamBuffer != null) {
        this.debug('Appending previously held buffer: ', this._pendingTokenStreamBuffer);
        this._stream.prepend(this._pendingTokenStreamBuffer);
        this._tokenStreamRemainingLength += this._pendingTokenStreamBuffer.length;
        this._pendingTokenStreamBuffer = null;
      }
    } catch (err) {
      if (err instanceof StreamIndexOutOfBoundsError) {
    ...
    if (this._tokenStream != null) {
      ...
      this.debug('Buffer remaining: ', this._stream.getBuffer());
  ...
  TdsClient.prototype._socketEnd = function() {
    var _ref2;
    this.debug('Socket ended remotely');
    this._socket = null;
    this.state = TdsConstants.statesByName['INITIAL'];
    return (_ref2 = this._handler) != null ? typeof _ref2.end === "function" ? _ref2.end() : void 0 : void 0;
  };

  TdsClient.prototype._socketClose = function(had_error) {
    var _ref2;
    this.debug('Socket closed');
    this._socket = null;
    this.state = TdsConstants.statesByName['INITIAL'];
    return (_ref2 = this._handler) != null ? typeof _ref2.close === "function" ? _ref2.close(had_error) : void 0 : void 0;
  };
